  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-8"
      ...
      <div className="w-full max-w-md">
        ...
        {step === 'loading' && (
          <div className="border-2 border-white/20 p-8 text-center text-white/40 text-xs tracking-widest animate-pulse">
            VERIFYING INVITE...
          </div>
        )}
        ...
        {step === 'error' && (
          <div className="border-2 border-red-400/40 p-8 text-center space-y-4">
            ...
            <p className="text-white/50 text-xs leading-relaxed">{errorMsg}</p>
            <button onClick={() => router.push('/')}
              className="text-xs border border-white/20 px-4 py-2 hover:border-white/50 hover:text-white transition-all text-white/40">
              ← Go Home
            </button>
          </div>
        )}
        ...
        {step === 'form' && invite && (
          <div className="border-2 border-white">
            <div className="border-b border-white/20 px-6 py-4">
              <div className="text-xs text-white/40 tracking-widest uppercase mb-1">You&apos;ve been invited to</div>
              <div className="font-black text-lg tracking-wider">
                Workspace: <span className="border border-white/30 px-2 py-0.5 text-sm">{invite.workspace_id}</span>
              </div>
              {invite.invited_by && (
                <div className="text-white/30 text-xs mt-1">by {invite.invited_by}</div>
              )}
            </div>
            ...
            <form onSubmit={handleJoin} className="p-6 space-y-4">
              ...
              <div>
                <label className="text-white/40 text-xs tracking-widest uppercase block mb-2">YOUR EMAIL *</label>
                <input
                  type="email"
              ...
              <button
                ...
                className="w-full py-3 bg-white text-black font-black text-xs tracking-widest uppercase hover:bg-black hover:text-white border-2 border-white transition-all disabled:opacity-30 disabled:cursor-not-allowed mt-2"
              >
